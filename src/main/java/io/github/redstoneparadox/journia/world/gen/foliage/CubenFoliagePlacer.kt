...
class CubenFoliagePlacer(radius: Int, randomRadius: Int, offset: Int, randomOffset: Int): FoliagePlacer(radius, randomRadius, offset, randomOffset) {
    ...
    override fun generate(world: ModifiableTestableWorld, random: Random?, config: TreeFeatureConfig, trunkHeight: Int, treeNode: TreeNode, foliageHeight: Int, radius: Int, leaves: MutableSet<BlockPos>, i: Int, blockBox: BlockBox?) {
        ...
        if (treeNode is CubenTreeNode && treeNode.branch) {
            for (x in -1..1) {
                for (y in -1..1) {
                    for (z in -1..1) {
    ...
    class CubenTreeNode(center: BlockPos?, foliageRadius: Int, giantTrunk: Boolean, val branch: Boolean):
        TreeNode(center, foliageRadius, giantTrunk)

    companion object {
        val CODEC: Codec<CubenFoliagePlacer> = RecordCodecBuilder.create { instance ->
            ...
            ).apply(instance, ::CubenFoliagePlacer)
        }
    }
}